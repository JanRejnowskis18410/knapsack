...
func TestGetPerfectKnapsack(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotPerfectKnapsack := GetKnapsack(tt.args.items, tt.args.knapsackCapacity); !reflect.DeepEqual(gotPerfectKnapsack, tt.wantPerfectKnapsack) {
				t.Errorf("GetKnapsack() = %v, want %v", gotPerfectKnapsack, tt.wantPerfectKnapsack)
			}
		})
	}
}
